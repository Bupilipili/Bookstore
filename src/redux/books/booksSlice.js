...
export const fetchBooksAsync = createAsyncThunk('books/fetchBooks', async () => {
  const response = await axios.get(`${API_BASE_URL}/apps/ktMaPkuUjcdRtsd1h31t/books`);
...
export const addBookAsync = createAsyncThunk('books/addBook', async ({ book }) => {
  await axios.post(`${API_BASE_URL}/apps/ktMaPkuUjcdRtsd1h31t/books`, book);
  return book;
});

export const removeBookAsync = createAsyncThunk('books/removeBook', async ({ item_id }) => {
  await axios.delete(`${API_BASE_URL}/apps/ktMaPkuUjcdRtsd1h31t/books/${item_id}`);
  return item_id;
});
